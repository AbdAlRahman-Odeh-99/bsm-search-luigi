...
def generate_GenerateCommand(data):
    return f"""
        ...
        pwd
        python {code_dir}/generatetuple.py {data['type']} {data['nevents']} {data['output_file']}
    """
...
def select_data_genertion(option, suffix, region, variation):
    type = option['type']
    njobs = option['njobs']
    nevents = option['nevents']
    return{
...
def hist_shape_data_genertion(option, shapevar, weight, variations):
    type = option['type']
    return {
...
def hist_shape_GenerateCommand(data):
    return f"""
        ...
        source {thisroot_dir}/thisroot.sh        
        python {code_dir}/histogram.py {data['input_file']} {data['output_file']} {data['option']['type']} {data['weight']} {data['variations']} {data['name']}
        """
...
class Hist_Shape(luigi.Task):
    ...
    def requires(self):
        ...
        for select_option in select_options:
            data = select_data_genertion(option, select_option['suffix'], select_option['region'], select_option['variation'])
...
def hist_weight_data_genertion(option, shapevar, variations):
    type = option['type']
...
def merge_explicit_data_genertion(option, operation, variations): #, njobs, weight, ):
    ...
    if('mc' in type):
        ...
        input_files = base_dir + '/' + type + '_hist.root qcd_hist.root'
    elif('all' in type):
    ...
    def requires(self):
        ...
        if ('merge_hist_all' in operation):
            ...
            for shape_option in hist_shape_mc_options:
                data = hist_shape_data_genertion(option, shape_option['shapevar'], weight, variations)
...
if __name__ == '__main__':
    ...
    for key in mc_options.keys():
        ...
        njobs = option['njobs']
        weight = option['mcweight']
        variations = variations_merge_options[type]
    ...
    for key in signal_options.keys():
        ...
        type = option['type']
        weight = option['mcweight']
        variation = variations_merge_options[type]
